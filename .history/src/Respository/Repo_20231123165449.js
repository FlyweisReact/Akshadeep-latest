...
export const passenger_detail_foriegncurrency = async (id , formPayload) => {
  try{
    const res = await axios.put(`${localhost}foriegncurrency/${id}` , formPayload)

  }catch{}
}

export const get_challan = async (id) => {
  try{
    const res = await axios.get(`${localhost}foriegncurrency/655f22c40d041c3f5be52880`)
  }catch{}
}
